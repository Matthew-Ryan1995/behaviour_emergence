...
file_path = "../data/simulations/large_scale_Bstar_by_R0/"
...
seed = simulation_parameters["seed"] + (array_val)
...
def load_bstar_data(file, file_path="../data/simulations/large_scale_Bstar_by_R0/",
    ...
    try:
        ...
        # Parameter
        extracted_params = re.findall("_[0-9]*.[0-9]*",  file)
        b_star = np.float32(re.sub("_", "", extracted_params[0]))
        r_0 = np.float32(re.sub("_", "", extracted_params[2]))
    ...
    tmp_params["transmission"] = r_0/tmp_params["infectious_period"]
    w1 = get_w1(b_star, tmp_params)
    tmp_params["B_social"] = w1

    I0 = params["I0"]
    B0 = int(b_star * params["P"]) - params["I0"]
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(mc_cores) as p:
        df = list(p.map(load_bstar_data, filenames))
    # df = list(map(load_bstar_data, filenames))
    df = pd.DataFrame(df)

    df = df.sort_values(by=["Bstar", "R0"]).reset_index()

    df.to_csv(f"../data/df_results2_Bstar_by_R0_seed_{seed}.csv")

# %%
# file_path = "../data/simulations/Bstar_by_R0/"
