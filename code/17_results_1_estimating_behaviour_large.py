...
# %% Get and laod data
file_path = "../data/simulations/large_baseline/"
...
for ii in range(20):
    seed += 1
    target_file = f"seed_{seed}"

    filenames_tmp = [f for idx, f in enumerate(filenames) if target_file in f]

    with gzip.open(file_path + filenames_tmp[0], "rb") as f:
    ...
    results_list.append(gillespy2.core.jsonify.Jsonify.from_json(results_json))
...
plt.savefig(f"../figs/results1_{save_name}_timeseries_large.png",
            dpi=dpi,
            bbox_inches="tight")
# plt.close()
plt.show()
...
n, bins = np.histogram(B_snapshot, bins=30)
y_coord = 0.8*n.max()

plt.plot([np.median(B_median), np.median(B_median)], [0, y_coord], "blue",
         linewidth=linewidth)
...
plt.savefig(f"../figs/results1_{save_name}_behaviour_snapshot_large.png",
            dpi=dpi,
            bbox_inches="tight")
# plt.close()
plt.show()
...
# plt.legend(["Behaviour", "Infection"])
# plt.title("Dashed line - exponential\nDotted line - Cubic")
plt.savefig(f"../figs/supp_results1_{save_name}_timeseries_large.png",
            dpi=dpi,
            bbox_inches="tight")
# plt.close()
plt.show()
...
plt.plot([np.median(B_median), np.median(B_median)], [0, y_coord], "grey",
         linewidth=linewidth)
...
plt.savefig(f"../figs/supp_results1_{save_name}_behaviour_snapshot_large.png",
            dpi=dpi,
            bbox_inches="tight")
# plt.close()
