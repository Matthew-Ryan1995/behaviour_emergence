...
error_type = ["exp", "poly"]
day = [5, 10, 15]
file_path = f"../data/df_results2_Bstar_by_R0_seed_{seed}.csv"
df = pd.read_csv(file_path, index_col=0)
for i in range(1, 20):
    seed_tmp = seed + i
    file_path = f"../data/df_results2_Bstar_by_R0_seed_{seed_tmp}.csv"
    ...
    for d in day:
        for e in error_type:
            df[f"{e}_error_{d}"] += df2[f"{e}_error_{d}"]

df["pHat"] /= 20
df["FS_avg"] /= 20
df["FS_conditional"] /= 20
...
dpi = 600
font_size = 16
lvls = [0.0, 0.15, 0.3, 0.45, 0.6, 0.75, 0.9, 1.0]
...
plt.savefig("../figs/results2a_outbreak_prob_large.png",
...
plt.savefig("../figs/results2a_final_size_large.png",
            dpi=dpi, bbox_inches="tight")
# plt.close()
plt.show()
...
# plt.show()
fig.savefig("../figs/results2a_error_large.png", dpi=dpi, bbox_inches="tight")
# plt.close(fig)
plt.show()
