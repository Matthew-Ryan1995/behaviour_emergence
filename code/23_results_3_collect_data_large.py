...
file_path = "../data/simulations/large_interventions/"
...
baseline_path = "../data/simulations/large_baseline/"
...
def load_intervention_data(file, file_path="../data/simulations/large_interventions/",
...
def load_baseline_data(file, file_path="../data/simulations/large_baseline/",
                       P=simulation_parameters["P"]):
    # Load data
    # with open(file_path + file, "r") as f:
    #     results_json = json.load(f)
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(mc_cores) as p:
        df = list(p.map(load_intervention_data, filenames))
    df = pd.DataFrame(df)

    df = df.sort_values(by=["target", "day", "strength"])

    df.to_csv(f"../data/df_results3_intervention_seed_{seed}.csv")
# %%
    df_base = list(map(load_baseline_data, baseline_files))
    df_base = pd.DataFrame(df_base)

    df_base.to_csv(f"../data/df_results3_baseline_seed_{seed}.csv")
