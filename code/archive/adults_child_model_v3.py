...
def eqns_to_solve(x):
    ...
    beta_cc2 = beta_cc * N_c / P
    beta_ac2 = beta_ac * N_c / P
    beta_ca2 = beta_ca * N_a / P
    beta_aa2 = beta_aa * N_a / P

    child_denom = beta_cc2 + beta_ca2 + gamma
    adult_denom = beta_ac2 + beta_aa2 + gamma

    cc_val = beta_cc2 / child_denom
    ca_val = beta_ca2/child_denom
    c_gamma = gamma/child_denom

    ac_val = beta_ac2/adult_denom
    aa_val = beta_aa2/adult_denom
...
def bad_ctmc(param_vals, P=100, I0=1, B0=1, t_end=100):
    ...
    beta_cc = gillespy2.Parameter(
        name="beta_cc", expression=param_vals["beta_cc"] / P)
    beta_ac = gillespy2.Parameter(
        name="beta_ac", expression=param_vals["beta_ac"] / P)
    beta_ca = gillespy2.Parameter(
        name="beta_ca", expression=param_vals["beta_ca"] / P)
    beta_aa = gillespy2.Parameter(
        name="beta_aa", expression=param_vals["beta_aa"] / P)
...
for index in range(0, num_trajectory):
    ...
    if (trajectory["Sc"][-1] + trajectory["Sa"][-1]) > int(P*0.9):
        count += 1
