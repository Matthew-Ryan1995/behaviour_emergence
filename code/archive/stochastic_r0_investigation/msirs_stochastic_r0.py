...
def run_iterations(model):
    ...
    while (T[count] < ND) and (res[-1, 2:4].sum() > 0):
        ...
        res = run_mod.P.reshape(1, 6)

        if "recover" in event:
            break

    return res[-1, 2:4].sum(), T
...
#     plt.show()
    ...
    for x in range(iteration_runs):
        ...
        PP = np.array([1e4/2, 1e4/2, 1, 0, 0, 0])

        R0 = 5
        gamma = 0.4

        args = {"beta": R0*gamma, "gamma": gamma, "P": PP, "t": 0,
                "mask_c": 0.8, "mask_p": 0.8, "a3": 0, "w3": 0, "w1": 0.6,
                "a2": (0.6 - 0.5)/2
                }
    ...
    Delta = ((mod.w1 - mod.a1 + mod.w3 + mod.a2 + mod.a3) - np.sqrt((mod.w1 - mod.a1 + mod.w3 +
             mod.a2 + mod.a3)**2 - 4 * (mod.w1 - mod.a1) * (mod.a2 + mod.a3))) / (2 * (mod.w1 - mod.a1))

    x = mod.a1 * Delta + mod.a2
    y = mod.w1 * (1 - Delta)

    a = (1-args["mask_p"])*(gamma + y) + x
    b = (1 - args["mask_p"]) * y + gamma + x
    Gamma = (R0*gamma)/(gamma * (gamma + x + y))

    R0_est = Gamma * ((1-args["mask_c"]) * (1-Delta) * a + Delta * b)
    print(f"Best guess R0 is {R0_est}")
